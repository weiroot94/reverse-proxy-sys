...
class TcpTransferHandler extends ChannelInboundHandlerAdapter {
    ...
    private void connectToRemote(ChannelHandlerContext ctx) throws UnknownHostException, UnknownHostException {
        ...
        switch (addr) {
            case V4 -> connectFuture = bootstrap.connect(new InetSocketAddress(address, port));
            case V6 -> {
            ...
            case DOMAIN -> connectFuture = bootstrap.connect(new InetSocketAddress(address, port));
            default -> {
